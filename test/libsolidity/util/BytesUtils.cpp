...
std::string BytesUtils::formatFixedPoint(bytes const& _bytes, bool _signed, size_t _fractionalDigits)
{
	...
	if (_signed)
	{
		s256 signedValue{u2s(u256{_bytes})};
		if (signedValue < 0)
		{
			negative = true;
...
string BytesUtils::formatRawBytes(
	bytes const& _bytes, solidity::frontend::test::ParameterList const& _parameters, string _linePrefix)
{
	stringstream os;
